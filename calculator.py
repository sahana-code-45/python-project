...
def handle_button_click(clicked_button_text):
    ...
    if clicked_button_text == "=":
        ...
        try:
            #Replace custom symbol with python operator

            expression = current_text.replace("÷","/").replace("x","*")
            result = eval(expression)
            

            #check if the result is the whole number
        ...
        #Convert the current number to a decimal dividing it by 1c00
        ...
        #convert the current number to the -ve number
...
#create buttons and add them to the grid using loop
